...
namespace BattleInfoPlugin.Models
{
	...
	public class EnemyDataProvider
	{
		...
		public Formation GetNextEnemyFormation(map_start_next startNext)
		{
			...
			if (startNext.api_enemy == null) return Formation.없음;
		...
		public CellEvent GetNextCellInfo(map_start_next startNext)
		{
			this.Dump("GetNextCellInfo");

			if (startNext.api_event_id == 0) return CellEvent.없음;
			try
			{
				return (CellEvent)startNext.api_event_id;
			}
			catch
			{
				return CellEvent.오류;
		...
		public void UpdateMapData(map_start_next startNext)
		{
			...
			this.UpdateMapEvent(startNext);
			this.Save();
			this.Dump("UpdateMapData");
		}
		private void UpdateMapEvent(map_start_next startNext)
		{
			if (startNext.api_event_id == null) return;
			var eventInfo = startNext.api_event_id;
